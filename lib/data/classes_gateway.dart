...
abstract interface class ClassesGateway {
  ...
  Future<CourseClass> createClass({
    required String accessToken,
    required CourseClassInput input,
  });
  Future<CourseClass> updateClass({
    required String accessToken,
...
class HttpClassesGateway implements ClassesGateway {
  ...
  Future<List<CourseClass>> listClasses(String accessToken) async {
    ...
    return rows
        .whereType<Map>()
        .map((row) => CourseClass.fromJson(_normalize(row)))
        .toList();
  }
  ...
  ClassesApiException _malformed(int statusCode) => ClassesApiException(
    message: 'The NaviPet server returned an invalid class response.',
